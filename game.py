...
from player import Player # Player untuk di laut (dalam perahu)
from boat import Boat
from game_map import GameMap # Peta Laut
from ui import UI
from inventory import Inventory
from market import Market
from map_explore import MapExplorer # Eksplorasi Peta Laut
from fishing_system import FishingSystem
from fishing_challenge import FishingSkillChallenge
from land_explorer import LandExplorer # Impor LandExplorer
...
class Game:
    def __init__(self, screen):
        ...
        self.land_explorer = None 
        self.map_explorer = None 
    ...
    def change_state(self, new_state_name, data=None):
        ...
        if new_state_name == 'land_explore': 
            ...
            print("--- Game: Menginisialisasi LandExplorer... ---")
            try:
                self.land_explorer = LandExplorer(self) 
                print("--- Game: LandExplorer berhasil diinisialisasi. ---")
            except Exception as e:
                print(f"--- Game: ERROR saat menginisialisasi LandExplorer: {e} ---")
                import traceback; traceback.print_exc()
                self.current_state_name = old_state
                if old_state != 'main_menu': self.change_state('main_menu')
                else: self.running = False 
                return
            # land_music_path = os.path.join(self.config.SOUND_PATH, "daratan_theme.ogg")
            # self.play_music_file(land_music_path, loop=-1)

        elif new_state_name == 'map_explore': 
            print("--- Game: Memproses state map_explore (laut) ---")
            print("--- Game: Menginisialisasi MapExplorer (laut)... ---")
            try:
                self.map_explorer = MapExplorer(self)
                print("--- Game: MapExplorer (laut) berhasil diinisialisasi. ---")
            except Exception as e:
                print(f"--- Game: ERROR saat menginisialisasi MapExplorer (laut): {e} ---")
                import traceback; traceback.print_exc()
                self.current_state_name = old_state 
                if old_state == 'land_explore': self.change_state('land_explore') 
                elif old_state != 'main_menu': self.change_state('main_menu')
                else: self.running = False 
                return
            # sea_map_music_path = os.path.join(self.config.SOUND_PATH, "map_laut_theme.ogg")
            # self.play_music_file(sea_map_music_path, loop=-1)
