...
class Game:
    def __init__(self, screen):
        ...
        # self.desired_waterline_on_screen_y = 432 # Nilai Asli
        # self.desired_waterline_on_screen_y = 502 # Percobaan 1
        # self.desired_waterline_on_screen_y = 532 # Percobaan 2
        self.desired_waterline_on_screen_y = 400 # PERUBAHAN: Nilai baru yang lebih rendah lagi (misal: 532 + 50)
        ...
        if self.map_explorer and hasattr(self.map_explorer, 'player_map_rect') and self.map_explorer.player_map_rect:
    ...
    def change_state(self, new_state_name, data=None):
        ...
        if new_state_name == 'land_explore':
            ...
            # Panggil setup_scene untuk map_explorer jika ada dan diperlukan
            if self.map_explorer and hasattr(self.map_explorer,'setup_scene'): 
                 # Jika map_explorer perlu di-refresh setiap kali masuk state, panggil setup_scene
                 # self.map_explorer.setup_scene() # Opsional, tergantung kebutuhan
                 pass # Untuk sekarang, tidak memanggil setup_scene agar posisi kapal tidak reset
            ...
            if data and 'location_name' in data:
                ...
                try:
                    ...
                    self.water_bottom_y_world = self.boat.rect.bottom + (self.config.SCREEN_HEIGHT * 0.7) 
                    self.water_bottom_y_world = min(self.water_bottom_y_world, fishing_world_height - 20)
                    if self.water_top_y_world >= self.water_bottom_y_world: 
    ...
    def spawn_visible_fish(self, amount=5):
        ...
        for _ in range(amount): 
            ...
            if fish_data: 
                ...
                # Panggil __init__ Fish dengan config
                new_fish_sprite = Fish(fish_data, (spawn_x, spawn_y), self.config)
                if new_fish_sprite.image: # Hanya tambahkan jika gambar berhasil dimuat
                    self.visible_fish_sprites.add(new_fish_sprite)
                else:
                    print(f"--- Game WARN: Gagal membuat sprite untuk ikan {fish_data.get('name')}, gambar tidak valid.")
    ...
    def run(self):
        ...
        # Panggil change_state awal untuk mengatur scene yang dimuat atau dari save file
        # Jika current_state_name sudah diatur dari load_game, panggil change_state untuk setup scene nya
        if self.current_state_name:
            self.change_state(self.current_state_name)
        else: # Fallback jika tidak ada state dari save file (seharusnya tidak terjadi dengan implementasi GameData)
        ...
        while self.running:
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT: 
                    self.running = False; self.game_data_manager.save_game() 
    ...
    def render_current_state(self):
        ...
        if active_renderer and hasattr(active_renderer, 'render'): active_renderer.render(self.screen)
            ...
            if self.current_game_map and self.current_game_map.background_image and self.fishing_camera: 
                ...
                bg_img_width = bg_img.get_width(); bg_img_height = bg_img.get_height()
                if bg_img_width > 0 and self.boat and self.boat.rect: 
                    waterline_world_y = self.boat.rect.bottom 
                    horizon_offset_in_bg_image = bg_img_height * 0.78 
            ...
            if self.fishing_camera: 
                for fish_sprite in self.visible_fish_sprites:
                    if fish_sprite.image and fish_sprite.rect: # Pastikan image dan rect ada
        ...
        if self.config.DEBUG: 
            debug_start_y = self.config.SCREEN_HEIGHT - (self.debug_font.get_height() + 3) * 7 # Tambah ruang untuk debug baru
