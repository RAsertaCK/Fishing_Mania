        ...
        if font_name_to_load and font_name_to_load.strip() != "" and font_name_to_load.lower() != 'none':
            ...
            if os.path.exists(potential_path):
                actual_font_path = potential_path
                 ...
                 print(f"--- Menu ({self.__class__.__name__}): Font '{font_name_to_load}' tidak ditemukan di '{self.config.FONT_PATH}'. Akan coba sebagai SysFont.")
...
class MainMenu(Menu):
    def __init__(self, game):
        # PERUBAHAN: Menggunakan "TampilanAwal.png" untuk MainMenu juga
        super().__init__(game, background_image_filename="TampilanAwal.png")
        self.title = "Fishing Mania"
        self.options = [
            ("Mulai Petualangan", lambda: self.game.change_state('map_explore')),
...
class ShopMenu(Menu):
    ...
    def attempt_upgrade(self, upgrade_type):
        ...
        if cost is not None and self.game.wallet >= cost:
            ...
            if self.game.boat.upgrade(upgrade_type): print(f"    {upgrade_type.capitalize()} berhasil di-upgrade!")
            else: self.game.wallet += cost
        self.update_options()
...
class MarketScreen(Menu):
    ...
    def update_options(self):
        ...
        self.options = [ (sell_all_text, self.sell_all_fish if num_fish > 0 else None), ("Kembali", lambda: self.game.change_state('main_menu')) ]
