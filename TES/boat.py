...
class Boat:
    ...
    def __init__(self, game_map, config_instance):
        ...
        if self.game_map and hasattr(self.game_map, 'name') and self.game_map.name not in ["initial_setup", "dummy"]:
            ...
            self.base_image = pygame.Surface((1,1), pygame.SRCALPHA)
            self.rect = self.base_image.get_rect(center=(-10000, -10000)) 

    def load_sprite(self):
        if not self.type or self.type in ["initial_setup", "dummy", "default"]:
            if not self.rect or (self.base_image and self.base_image.get_width() <= 1) : 
                placeholder_surface = pygame.Surface((100,50), pygame.SRCALPHA)
                placeholder_surface.fill(self.config.COLORS.get('blue', (0,0,255,100)))
                self.base_image = placeholder_surface
        ...
        try:
            image_path = os.path.join(self.config.BOAT_PATH, f"boat_{self.type}.png")
            self.base_image = self.config.load_image(image_path, scale=0.7)
            self.rect = self.base_image.get_rect(
                midbottom=(self.config.SCREEN_WIDTH // 2, self.config.SCREEN_HEIGHT - 50)
            )
        except Exception as e:
            print(f"      ERROR saat load_sprite untuk boat tipe {self.type}: {e}")
            fallback_path = os.path.join(self.config.BOAT_PATH, "boat_default.png")
            try:
                self.base_image = self.config.load_image(fallback_path, scale=0.7)
            except: 
    ...
    def change_map(self, new_game_map):
        ...
        if self.game_map and hasattr(self.game_map, 'name') and self.game_map.name not in ["initial_setup", "dummy"]:
            ...
            if not self.rect: 
                ...
                self.rect.midbottom=(self.config.SCREEN_WIDTH // 2, self.config.SCREEN_HEIGHT - 50)
    ...
    def update(self, dt, keys):
        if not self.rect or self.type in ["initial_setup", "dummy"]: 
            return 
    ...
    def render(self, surface):
        if self.base_image and self.rect and self.type not in ["initial_setup", "dummy"] and self.base_image.get_width() > 1 : 
            surface.blit(self.base_image, self.rect)
        
        if self.upgrades.get("sonar", 0) > 0 and self.rect and self.type not in ["initial_setup", "dummy"]:
            try:
